   ...
   <div>
    <v-container grid-list-xl fluid>
      <v-layout row wrap>
        ...
        <v-spacer></v-spacer>
        <v-btn depressed outline icon fab dark color="primary" small @click="$router.push({ name: 'EditUser', params: { id: $route.params.id }})">
          <v-icon>edit</v-icon>
        </v-btn>
        <v-btn color="primary" flat @click="$router.push({ name: 'pengaturan/user' })"><v-icon>list</v-icon>&nbsp;list</v-btn>
        <v-flex sm12>
            ...
            <div slot="widget-content">
              <v-container>
                ...
                <v-layout align-center row spacer slot="header">
                  ...
                  <v-flex xs8 sm10 md9 >
                    <p class="font-weight-regular">{{ user.birthplcae }}, {{ user.birthdate_formatted }}</p>
                  </v-flex>
                </v-layout>
                <v-layout align-center row spacer slot="header">
                  <v-flex xs4 sm2 md3>
